...
#include <skynet/phase1.h>

#include "skynet_ref/verifier.hpp"
...
//		std::cout << " (" << proof.size() << " x 32-bit)" << std::endl;
		...
		skynet::Bits(y, 32).ToBytes(challenge);
		
		uint8_t proof_bytes[256] = {};
		{
			size_t i = 0;
			for(auto x : proof) {
				skynet::Bits(x, 32).ToBytes(proof_bytes + (4 * i++));
			}
		}
		{
			skynet::LargeBits bits(proof_bytes, sizeof(proof_bytes), sizeof(proof_bytes) * 8);
//			std::cout << "proof = " << bits.ToString() << std::endl;
		}
		
		skynet::Verifier verify;
